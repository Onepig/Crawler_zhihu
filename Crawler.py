...
class Question:
    ...
    def parser(self):
        global session
        if session == None:
            create_session();#模拟登陆返回数据报
        s = session
        r = s.get(self.url) #使用get方法
        soup = BeautifulSoup(r.content)# 返回解析后的html或xml文件
        self.soup = soup

    def get_title(self):
        if hasattr(self, "title"):#实例是否存在一个title数据
            ...
            if title is not None:
                ...
                print "Question (%s) in not found!"%self.url
            ...
            if followees_num == 0:
                ...
                for i in range((followees_num - 1) / 20 + 1):
                    if i == 0:
                        ...
                        offset = i * 20
                        if len(re.findall("hash_id&quot;: &quot;(.*)&quot;},", r.text)) != 0: # ??????
...
class Answer:
    ...
    def to_txt(self):
        ...
        if self.get_author().get_user_id() == anon_user_id:
            ...
            #     print file_name
            if os.path.exists(os.path.join(os.path.join(os.getcwd(), "text"), file_name)):
                os.remove(os.path.join(os.path.join(os.getcwd(), "text"), file_name))
...
class Topic:
    ...
    def __init__(self, topic_url, name = None):

        if topic_url[0:len(topic_url) - 8] != "http://www.zhihu.com/topic/":
    ...
    def get_name(self):
        if hasattr(self, 'name'):
            ...
            if self.soup is None:
                self.__parser()
            soup = self.soup
            self.name = soup.find("h1", class_="zm-editable-content").string.encode("utf-8").strip()
            if platform.system() == 'Windows':
                return self.name.decode('utf-8').encode('gbk')
            return self.name

    def get_fatherTopic(self):
        if self.soup is None:
            self.__parser()
        soup = self.soup

        father_topics = soup.find("div", class_="zm-side-section-inner parent-topic").find_all('a',class_='zm-item-tag') #.string.encode("utf-8").strip()
        '''print type(father_topics) # the type is Tag
        '''
        if father_topics is None:
            print "%s is the root topic."%(self.get_name())
        else:
            for father_topic in father_topics:
                fathertopic_name=father_topic.string.encode("utf-8").strip()
                if platform.system() == 'Windows':
                    yield Topic("http://www.zhihu.com"+father_topic["href"],fathertopic_name)
                yield Topic("http://www.zhihu.com"+father_topic["href"],fathertopic_name)

    def get_childrenTopic(self):
        if self.soup is None:
            self.__parser()
        soup = self.soup
        children_topics= soup.find("div", class_='zm-side-section-inner child-topic').find_all('a',class_='zm-item-tag') # .string.encode("utf-8").strip()
        if children_topics is None:
            print "%s is the lowest topic"%self.get_name()
        for child_topic in children_topics:
            childtopic_name=child_topic.string.encode("utf-8").strip()
            if platform.system() == 'Windows':
                yield Topic("http://www.zhihu.com"+child_topic["href"],childtopic_name)
            yield Topic("http://www.zhihu.com"+child_topic["href"],childtopic_name)
    ...
    def get_all_questions(self):
        ...
        for i in range(int(total_pages_nums)):
            single_page_url=self.topic_url+"/questions?page=%d"%(i+1) # the url of questions in page (i+1)
    ...
    def get_followers_num(self):
        if self.soup is None:
            self.__parser()
        soup = self.soup
        followers_num = soup.find("div", class_="zm-topic-side-followers-info").a.string.encode("utf-8").strip()
        if platform.system() == 'Windows':
            return followers_num.decode('utf-8').encode('gbk')
        return followers_num

    def get_all_topic_followers(self):
        global session
        if session is None:
            create_session()
        s = session
        followers_num=int(self.get_followers_num())
        all_followers_url=self.topic_url+"/followers"
	print all_followers_url
        r=s.get(all_followers_url)
        soup=BeautifulSoup(r.content)

        for i in range((followers_num - 1) / 40 + 1):
            if i == 0:
                user_url_list = soup.find_all("h2", class_= "zm-list-content-title")
                print len(user_url_list)
		for j in range(min(followers_num, 40)):
                    yield User("http://www.zhihu.com"+user_url_list[j].a["href"], user_url_list[j].a.string.encode("utf-8"))
            else:
                post_url = "http://www.zhihu.com/node/ProfileFollowersListV2"
                _xsrf = soup.find("input", attrs = {'name': '_xsrf'})["value"]
                offset = i * 40
		...
		f.close()
		print len(re.findall("javascript:;",r.text))
		print re.findall("javascript:;", r.text)
        '''
		if len(re.findall("javascript:;", r.text)) != 0:
                    hash_id = re.findall("javascript:;", r.text)[0]
                params = json.dumps({"offset": offset,"order_by":"created","hash_id": hash_id})
                data = {
                    '_xsrf': _xsrf,
                    'method': "next",
                ...
                header = {
                    'User-Agent': "Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:34.0) Gecko/20100101 Firefox/34.0",
                    'Host': "www.zhihu.com",
                    'Referer': all_followers_url
                }
                r = s.post(post_url, data = data, headers = header)
                follower_list = r.json()["msg"]
                for j in range(min(followers_num - i * 40, 40)):
                    follower_soup = BeautifulSoup(follower_list[j])
                    user_link = follower_soup.find("h2", class_ = "zm-list-content-title").a
                    yield User(user_link["href"], user_link.string.encode("utf-8"))
